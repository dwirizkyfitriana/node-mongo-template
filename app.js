...
if (process.env.NODE_ENV && process.env.NODE_ENV === 'development') {
    redisClient = redis.createClient(process.env.APPS_REDIS_HOST)
} else {
    redisClient = redis.createClient({
        host: process.env.APPS_REDIS_HOST,
        port: process.env.APPS_REDIS_PORT,
        password: process.env.APPS_REDIS_PASSWORD,
        db: Number(process.env.APPS_REDIS_DB_NUMBER),
        tls: {
            host: process.env.APPS_REDIS_HOST,
            port: process.env.APPS_REDIS_PORT,
            servername: process.env.APPS_REDIS_HOST
        }
    })
}
...
app.use(session({
    ...
    cookie: {
        ...
        // secure: true, // Set this to true only after application has a ssl enabled site
    }
}))
